...
EVID=ROOT/"reports"/"evidencias"/"HOT-INV-DEF-005"
...
("09","Fora da Garantia + Reparo Reprovado","Selecionar Garantia = NÃO. Anexar 'Orcamento fora da garantia.pdf'. Após o upload, selecionar Reparo aprovado = NÃO.","Campo Estado da máquina exibido.",["19_fora_da_garantia_orcamento_exibido.png","20_orcamento_anexado_reparo_aprovado_exibido.png","21_reparo_reprovado_estado_maquina_exibido.png"]),
("10","Máquina Não Funcional","Selecionar Estado da máquina = Máquina não funcional e clicar em Salvar.","STATUS: REPARO NAO AUTORIZADO — Equipamento com Defeito e Não Funcional.",["22_maquina_nao_funcional_antes_salvar.png","23_status_final_reparo_nao_autorizado_nao_funcional.png","99_teste_finalizado_com_sucesso.png"])
]
...
def image(n):
    if not (EVID/n).exists():
        return '<div class="missing">Evidência não encontrada: '+html.escape(n)+'</div>'
    return '<figure><a href="'+html.escape(n)+'"><img src="'+html.escape(n)+'"></a><figcaption>'+html.escape(n)+'</figcaption></figure>'

def main():
    EVID.mkdir(parents=True,exist_ok=True)
    st=status(); d=data(); cards=[]
