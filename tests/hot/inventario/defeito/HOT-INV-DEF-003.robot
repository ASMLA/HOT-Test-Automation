*** Settings ***
Documentation    DEF-003 - Defeito Fora Da Garantia + Reparo Aprovado.
...
${EVIDENCE_SCENARIO}   HOT-INV-DEF-003
${EVIDENCE_DIR}        ${EVIDENCE_ROOT}${/}${EVIDENCE_SCENARIO}

*** Test Cases ***
HOT-INV-DEF-003 - Defeito Fora Da Garantia Com Reparo Aprovado
    [Tags]    HOT    INVENTARIO    DEFEITO    DEF-003    FORA-GARANTIA    REPARO-APROVADO    E2E

    ${massa}=    Gerar Massa Do Ativo DEF-003
    Log    Serial gerado para toda a execução: ${massa}[serial]
    Log    Número do ativo gerado para toda a execução: ${massa}[numero_ativo]
    Log    Número do pedido gerado para toda a execução: ${massa}[numero_pedido]
    Log    Remessa do reparo gerada para toda a execução: ${massa}[remessa_reparo]
    ...
    Informar Equipamento Fora Da Garantia Com Reparo Aprovado    ${massa}    ${ORCAMENTO}
